...
The filesystem is built using FUSE, implemented in C using the `libfuse(FUSE)` library. The mail connection is done using `IMAP` (for reading/manipulating mailboxes) and `SMTP` (for writing), implemented using `libcurl` library.
...
Each folder in the mailbox (line INBOX, Drafts, Starred, or labels like TO_FOLLOWUP, etc) are mapped to directories underthe mount point (which acts as a relative root).
